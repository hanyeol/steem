...
BOOST_AUTO_TEST_CASE( open_and_create )
{
   ...
   try {
      ...
      // Opening without specifying a size creates an empty mapped file and fails; allow the
      // underlying interprocess exception here.
      BOOST_CHECK_THROW( db.open( temp ), boost::interprocess::interprocess_exception ); /// temp does not exist

      db.open( temp, 0, 1024*1024*8 );

      chainbase::database db2; /// open an already created db
      db2.open( temp );
      BOOST_CHECK_NO_THROW( db2.add_index< book_index >() ); /// ensure shared segment is usable read-only

      db.add_index< book_index >();
      // Adding the same index twice in the same database is undefined; avoid double-add.
