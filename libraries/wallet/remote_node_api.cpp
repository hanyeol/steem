 ...
 * @return Chain parameters including account creation fee, maximum block size, SBD interest rate, etc.
 */
...
/**
 * @brief Retrieves the current median of STEEM/SBD price feed
 * @return Median of price feeds provided by witnesses (STEEM/SBD exchange rate)
 */
...
/**
 * @brief Retrieves pending SBD to STEEM conversion requests for an account
...
{
   database_api::find_sbd_conversion_requests_args args{ .account = account };
   return send_call( "database_api", "find_sbd_conversion_requests", fc::variant(args) )
      .as<database_api::find_sbd_conversion_requests_return>().requests;
}
 ...
 * @param query Discussion query parameters (must specify account in tag field)
 * @return Vector of discussions authored or resteemed by the account
 */
 ...
 * @param query Discussion query parameters
 * @return Vector of discussions ordered by promotion (burned SBD for visibility)
 */
 ...
 * @return Volume data for STEEM and SBD traded in the last 24 hours
 */
