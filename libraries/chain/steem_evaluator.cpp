   ...
   const dynamic_global_property_object& props, bool mined, const account_name_type& recovery_account, uint32_t hardfork )
{
   ...
   if( recovery_account != STEEM_TEMP_ACCOUNT )
   {
...
void delete_comment_evaluator::do_apply( const delete_comment_operation& o )
{
   ...
   if( comment.parent_author != STEEM_ROOT_POST_PARENT )
   {
...
void account_witness_vote_evaluator::do_apply( const account_witness_vote_operation& o )
{
   ...
   if( itr == by_account_witness_idx.end() ) {
      ...
      FC_ASSERT( voter.witnesses_voted_for < STEEM_MAX_ACCOUNT_WITNESS_VOTES, "Account has voted for too many witnesses." );
...
{ try {
   ...
   if( itr == comment_vote_idx.end() )
   {
      ...
      if( rshares > 0 )
      {
         FC_ASSERT( _db.head_block_time() < comment.cashout_time - STEEM_UPVOTE_LOCKOUT, "Cannot increase payout within last twelve hours before payout." );
      }
...
void convert_evaluator::do_apply( const convert_operation& o )
{
  ...
  auto steem_conversion_delay = STEEM_CONVERSION_DELAY;
...
#pragma message( "TODO: Update get_effective_vesting_shares when modifying this operation to support SMTs." )
   ...
   auto min_delegation = asset( wso.median_props.account_creation_fee.amount / 3, STEEM_SYMBOL ) * gpo.get_vesting_share_price();
   auto min_update = asset( wso.median_props.account_creation_fee.amount / 30, STEEM_SYMBOL ) * gpo.get_vesting_share_price();
