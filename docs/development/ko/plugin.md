...
API가 있는 새 플러그인을 만드는 데 필요한 대부분의 보일러플레이트를 포함하는 스켈레톤 구조는 `programs/util/newplugin.py`를 실행하여 자동 생성할 수 있습니다.
...
- 선택적 의존성에 대해 시작에서만 get/find_plugin을 호출하는 것이 안전합니다. 선택적 의존성은 명시적으로 지정되지 않습니다. 플러그인이 활성화된 경우 일부 선택적 동작일 뿐입니다. 선택적 의존성의 예는 `condenser_api`를 참조하십시오
