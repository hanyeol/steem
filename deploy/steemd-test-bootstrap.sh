...
STEEMD="/usr/local/steemd-test/bin/steemd"

UTILS="/usr/local/steemd-test/bin"
...
cp /etc/steemd/test.config.ini $HOME/config.ini
cp /etc/steemd/fastgen.config.ini $HOME/test_datadir/config.ini

chown steemd:steemd $HOME/config.ini
chown steemd:steemd $HOME/test_datadir/config.ini
...
# setup tinman
git clone https://github.com/hanyeol/tinman
cd tinman/tinman
git clone https://github.com/hanyeol/simple_steem_client
cd $HOME
...
echo steemd-test: starting fastgen node
...
echo steemd-test: pipelining transactions into fastgen node, this may take some time
( \
...
# let's get going
echo steemd-test: bringing up witness / full node
