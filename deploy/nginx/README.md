...
This directory contains NGINX configuration files for reverse proxying steemd WebSocket and HTTP API endpoints.
...
# Install steemd nginx config as global config
cp /etc/nginx/steemd.nginx.conf /etc/nginx/nginx.conf
```

### steemd-proxy.conf.template

NGINX site configuration template for proxying requests to steemd backend.

**Features:**
- Listens on port 8090 for incoming connections
- WebSocket connection proxy support
- Health check endpoint at `/health` (via fcgiwrap + steemd-healthcheck.sh)
...
```bash
echo "server 127.0.0.1:8091;" >> /etc/nginx/steemd-proxy.conf
echo "server 127.0.0.1:8092;" >> /etc/nginx/steemd-proxy.conf
echo "server 127.0.0.1:8093;" >> /etc/nginx/steemd-proxy.conf
echo "server 127.0.0.1:8094;" >> /etc/nginx/steemd-proxy.conf
echo "}" >> /etc/nginx/steemd-proxy.conf
```
...
This is the primary use case. The [steemd-paas-bootstrap.sh](../steemd-paas-bootstrap.sh) script automatically:

1. Backs up original `/etc/nginx/nginx.conf`
2. Copies `steemd.nginx.conf` to `/etc/nginx/nginx.conf` (global config)
3. Generates site config from `steemd-proxy.conf.template`
...
```bash
mv /etc/nginx/nginx.conf /etc/nginx/nginx.original.conf
cp deploy/nginx/steemd.nginx.conf /etc/nginx/nginx.conf
```

3. **Create site config from template:**
```bash
cp deploy/nginx/steemd-proxy.conf.template /etc/nginx/steemd-proxy.conf
echo "server 127.0.0.1:8091;" >> /etc/nginx/steemd-proxy.conf
echo "}" >> /etc/nginx/steemd-proxy.conf
```
...
Check steemd backend is running:
```bash
